...
public class HierarchialClusteringMotif
{
	...
	{		
		...
		{			
			...
			Vector<Motif> cluster = result.getCluster(c2);
			Motif m = MotifTools.getPerwiseIntersection(cluster);
			centers[c2] = m;
			centers[c1] = null;
			...
			{				
				...
				newD = 1.0 - centers[c2].sim(centers[k]);
				
				//newD = Math.pow(newD, 2.0);
	...
	public void initialize()
	{
		...
		centers = new Motif[result.data.size()];
		for (int i=0; i< result.numOfClusters; i++)
		{	
			result.dissimilarityMatrix.put(i,new Vector<Double>());
			centers[i] = new Motif((Motif)result.data.get(i).properties.firstElement());
			for (int j=0; j<i; j++)
			{
				Double d = (Double)distance.getDistance(result.data.get(i).properties, result.data.get(j).properties);
